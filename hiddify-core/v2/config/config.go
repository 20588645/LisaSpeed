...
func setRoutingOptions(options *option.Options, opt *HiddifyOptions) {
	...
	if opt.Region != "other" {
		...
		// Hardcoded domestic suffixes so split routing works even when the
		// remote geosite/geoip rule-set download from GitHub fails.
		if opt.Region == "cn" {
			cnSuffixes := []string{
				"moutaichina.com",
				"moutai.com",
	...
	if opt.EnableDNSRouting || opt.Region != "other" {
